...
RUN apt-get update && \
    ...
    mkdir -p $sbt_home && \
    wget -qO - --no-check-certificate https://github.com/sbt/sbt/releases/download/v$sbt_version/sbt-$sbt_version.tgz | tar xz -C $sbt_home --strip-components=1 && sbt sbtVersion
...
COPY --from=build-stage /home/builder/app/target/scala-2.12/messageApiService.jar /root
